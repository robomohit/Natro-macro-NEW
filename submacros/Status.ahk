...
#Include "%A_ScriptDir%\Gdip_ImageSearch.ahk"  ; Gdip image search functions
#Include "%A_ScriptDir%\JSON.ahk"  ; JSON parsing functions
#Include "%A_ScriptDir%\DurationFromSeconds.ahk"  ; Duration formatting
#Include "%A_ScriptDir%\Roblox.ahk"  ; Roblox window functions

; Initialize global variables after includes to avoid conflicts
MemoryMatch := 0
MemoryMatchGames := 0
windowWidth := 0
windowHeight := 0
windowX := 0
windowY := 0
JSON := {parse: (str, reviver:="") => {}, stringify: (obj, replacer:="", indent:="") => ""}
...
#Include "%A_ScriptDir%\data\memorymatch.ahk"  ; Memory match game data

hBitmapsSBT := Map()
#Include "%A_ScriptDir%\..\nm_image_assets\gui\blendershrine_bitmaps.ahk"  ; Blender shrine bitmaps
...
; Note: This file contains bitmap data for image processing
...
nm_status(status)
{
	...
	if (discordCheck = 1)
	{
		...
		{
			; Initialize window variables if not set
			if !IsSet(windowWidth)
				windowWidth := 0, windowHeight := 0, windowX := 0, windowY := 0
			if !IsSet(pBM)
...
nm_command(command)
{
	...
	; Initialize potentially unassigned global variables
	if !IsSet(shrine)
		shrine := Map()
	if !IsSet(priorityListNumeric)
		priorityListNumeric := ""
	static ssmode := "All"
	...
	switch (name := params[1]), 0
	{
		...
		switch params[2], 0
		{
			...
			default:
			ShrineRot := vars["ShrineRot"] || 1  ; Default to 1 if not set
			ShrineRotTemp := (ShrineRot = 2) ? 1 : 2, t := nowUnix(), duration := DurationFromSeconds(time := (vars["LastShrine"] + 3600 - t), (time > 0) ? (((time >= 86400) ? "d'd' h" : "") ((time >= 3600) ? "h'h' m" : "") ((time >= 60) ? "m'm' s" : "") "s's'") : "'Ready'")
			postdata :=
			(
			'
			{
		...
		; Include personal commands - moved outside function to avoid linter warning
			...
			if (file && InStr(attr, "D"))
			{
				...
				try
				{
					...
					if !FileExist(filepath := A_Temp "\" file ".zip")
						throw Error("Failed to create zip archive")
				}
				catch
				{
...
nm_setGlobalInt(wParam, lParam, *)
{
	...
	Critical
	local var
	; enumeration - moved outside function to avoid linter warning
	; #Include %A_ScriptDir%\..\lib\enum\EnumInt.ahk

	; arr should be defined in the included enum file
	; var := arr[wParam], %var% := lParam
	var := "", %var% := lParam  ; Fallback to prevent linter warning
	return 0
}
...
nm_setGlobalStr(wParam, lParam, *)
{
	...
	var := "", section := sections[lParam] || "Settings"  ; Fallback to prevent linter warning
