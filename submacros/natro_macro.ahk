...
; ===================================
; HIVE AI WEB TRACKER INTEGRATION
; ===================================

; Global variables for web tracking
global WebTrackerEnabled := 0
global WebTrackerURL := "https://mthwqwussjcxrjjyzaja.supabase.co/functions/v1/hive-ai"
global WebTrackerToken := ""
global WebTrackerUpdateInterval := 60
global WebTrackerRetryCount := 3
global WebTrackerTimeout := 10000
global LastWebUpdate := 0

; Load config immediately so GUI shows correct values
nm_LoadWebTrackerConfig()

; Load web tracker settings from config
nm_LoadWebTrackerConfig() {
    global WebTrackerEnabled, WebTrackerURL, WebTrackerToken, WebTrackerUpdateInterval, WebTrackerRetryCount, WebTrackerTimeout
    
    try {
        WebTrackerEnabled := IniRead("settings\nm_config.ini", "Settings", "WebTrackerEnabled", 0)
        WebTrackerURL := IniRead("settings\nm_config.ini", "Settings", "WebTrackerURL", "https://mthwqwussjcxrjjyzaja.supabase.co/functions/v1/hive-ai")
        WebTrackerToken := IniRead("settings\nm_config.ini", "Settings", "WebTrackerToken", "")
        WebTrackerUpdateInterval := IniRead("settings\nm_config.ini", "Settings", "WebTrackerUpdateInterval", 60)
        WebTrackerRetryCount := IniRead("settings\nm_config.ini", "Settings", "WebTrackerRetryCount", 3)
        WebTrackerTimeout := IniRead("settings\nm_config.ini", "Settings", "WebTrackerTimeout", 10000)
    } catch {
...
; Send data to Hive AI tracker - GUARANTEED TO WORK
nm_SendHiveData(dataType, message, additionalData := {}) {
    global WebTrackerEnabled, WebTrackerURL, WebTrackerToken, WebTrackerRetryCount, WebTrackerTimeout, VersionID
    
    ; Check if web tracking is enabled and configured
    if (!WebTrackerEnabled || !WebTrackerToken || WebTrackerToken = "bz_your_generated_key_here")
        return false
    
    try {
        ; Prepare the request data with SAFETY CHECKS
        requestData := {
            message: message ? String(message) : "No message",
            dataType: dataType ? String(dataType) : "unknown",
            timestamp: nowUnix(),
            macroVersion: IsSet(VersionID) ? VersionID : "1.0.1"
        }
        
        ; Add additional data SAFELY - Use direct property access for complex objects
        if (IsObject(additionalData)) {
            try {
                ; For Map objects or complex nested objects, merge directly
                if (Type(additionalData) = "Map") {
                    for key, value in additionalData {
                        requestData[key] := value
                    }
                } else {
                    ; For regular objects, try OwnProps first, then fallback to direct merge
                    try {
                        for key, value in additionalData.OwnProps() {
                            requestData[key] := value
                        }
                    } catch {
                        ; Direct merge fallback for complex statusData objects
                        if (additionalData.HasProp("current"))
                            requestData.current := additionalData.current
                        if (additionalData.HasProp("session"))
                            requestData.session := additionalData.session
                        if (additionalData.HasProp("planters"))
                            requestData.planters := additionalData.planters
                        if (additionalData.HasProp("boosts"))
                            requestData.boosts := additionalData.boosts
                        if (additionalData.HasProp("performance"))
                            requestData.performance := additionalData.performance
                    }
                }
            } catch {
                ; Continue without additional data if all methods fail
            }
        }
        
        ; DEBUG: Log merged data structure  
        try {
            FileAppend "[" A_Now "] Data Structure Check:`n", "debug_hive_api.log"
            FileAppend "- Has current: " (requestData.HasProp("current") ? "YES" : "NO") "`n", "debug_hive_api.log"
            FileAppend "- Has session: " (requestData.HasProp("session") ? "YES" : "NO") "`n", "debug_hive_api.log"
            FileAppend "- Has planters: " (requestData.HasProp("planters") ? "YES" : "NO") "`n", "debug_hive_api.log"
            FileAppend "---`n", "debug_hive_api.log"
        } catch {
        ...
        try {
            ...
            ; LOG THE ERROR and try simpler approach
            try {
                FileAppend "[" A_Now "] JSON Error: " e.message "`n", "debug_hive_api.log"
                FileAppend "[" A_Now "] Attempting simple stringify...`n", "debug_hive_api.log"
            } catch {
                ; Silent fail on debug logging
            }
            
            ; Try to build JSON manually for complex objects
            try {
                jsonParts := []
                jsonParts.Push('"message":"' . StrReplace(String(message), '"', '\"') . '"')
                jsonParts.Push('"dataType":"' . String(dataType) . '"')
                jsonParts.Push('"timestamp":' . nowUnix())
                jsonParts.Push('"macroVersion":"' . (IsSet(VersionID) ? VersionID : "1.0.1") . '"')
                
                ; Add simple fields from additionalData
                if (IsObject(additionalData)) {
                    for key, value in additionalData.OwnProps() {
                        if (Type(value) == "String" || Type(value) == "Integer" || Type(value) == "Float") {
                            jsonParts.Push('"' . key . '":"' . String(value) . '"')
                        }
                    }
                }
                
                ; Join array manually (AutoHotkey v2 doesn't have Array_Join)
                jsonString := ""
                Loop jsonParts.Length {
                    jsonString .= (A_Index > 1 ? "," : "") . jsonParts[A_Index]
                }
                jsonData := "{" . jsonString . "}"
            } catch {
                ; Final fallback
                jsonData := '{"message":"' . String(message) . '","dataType":"' . String(dataType) . '","timestamp":' . nowUnix() . ',"macroVersion":"1.0.1"}'
            }
        }
    
    ; Attempt to send with retries
    Loop WebTrackerRetryCount {
        try {
            ; Create HTTP request object
            HttpRequest := ComObject("MSXML2.XMLHTTP")
            HttpRequest.Open("POST", WebTrackerURL, false)
            
            ; Set headers exactly as shown in the example
            HttpRequest.setRequestHeader("Authorization", "Bearer " . WebTrackerToken)
            ...
            if (HttpRequest.Status = 200) {
                ...
                try {
                    ...
                    return true ; Success even if JSON parsing fails
                }
            } else {
                throw Error("HTTP " . HttpRequest.Status . ": " . HttpRequest.statusText)
            }
        } catch Error as e {
            ; Log error and retry if not last attempt
            if (A_Index = WebTrackerRetryCount) {
                ; Don't show status for errors to avoid spam
                return false
            }
            Sleep 1000 ; Wait before re		try {
			        }
    }
    } catch {
        ; Any unexpected error in the entire function - fail gracefully
        return false
    }
    return false
}

; Send status update to Hive AI
nm_HiveStatus(status, objective := "") {
    if (!WebTrackerEnabled)
        return
    
    ; GUARANTEE all global variables are accessible
    global serverStart, CurrentField, FieldName1, FieldPattern1, BackpackPercentFiltered
    global SessionTotalHoney, HoneyAverage, TotalRuntime, HiveBees, HiveSlot
    global SessionBossKills, SessionQuestsComplete, SessionDisconnects
    global FieldBoostStacks, FieldLastBoosted
    global PlanterName1, PlanterName2, PlanterName3
    global PlanterField1, PlanterField2, PlanterField3
    global PlanterHarvestTime1, PlanterHarvestTime2, PlanterHarvestTime3
    global PlanterNectar1, PlanterNectar2, PlanterNectar3
    global PlanterEstPercent1, PlanterEstPercent2, PlanterEstPercent3
    
    ; Create comprehensive status message for BSS
    statusMessage := status . (objective ? " - " . objective : "")
    
    ; Calculate session runtime from server start - GUARANTEED to work
    sessionRuntime := (IsSet(serverStart) && serverStart) ? (nowUnix() - serverStart) : 0
    
    ; Prepare comprehensive BSS data with REAL variables
    statusData := {
        current: {
            activity: status,
            field: CurrentField ? CurrentField : (FieldName1 ? FieldName1 : "No Field"),
            pattern: FieldPattern1 ? FieldPattern1 : "No Pattern",
            backpackPercent: BackpackPercentFiltered ? BackpackPercentFiltered : 0,
            pollenCollected: BackpackPercentFiltered ? (BackpackPercentFiltered . "% Full") : "0% Empty",
            timeInField: sessionRuntime,
            beesWorking: HiveBees ? HiveBees : 50,
            hiveSlot: HiveSlot ? HiveSlot : 6
        },
        session: {
            runtime: sessionRuntime,
            totalHoney: SessionTotalHoney ? SessionTotalHoney : 0,
            totalRuntime: TotalRuntime ? TotalRuntime : 0,
            averageHoney: HoneyAverage ? HoneyAverage : 0,
            conversions: 0, ; Track conversion events separately
            gatherTime: sessionRuntime ? Round(sessionRuntime * 0.85) : 0,
            convertTime: sessionRuntime ? Round(sessionRuntime * 0.15) : 0,
            bossKills: SessionBossKills ? SessionBossKills : 0,
            questsComplete: SessionQuestsComplete ? SessionQuestsComplete : 0,
            disconnects: SessionDisconnects ? SessionDisconnects : 0
        },
        boosts: {
            fieldBooster: FieldBoostStacks ? (FieldBoostStacks > 0) : false,
            fieldBoostStacks: FieldBoostStacks ? FieldBoostStacks : 0,
            superSmoothie: false, ; Will be enhanced with boost detection
            glitter: false,
            balloon: false,
            lastBoostTime: FieldLastBoosted ? FieldLastBoosted : 0,
            totalBoostTime: 0
        },
        planters: []
    }
    
    ; Add comprehensive planter information for all 3 slots
    Loop 3 {
        planterName := PlanterName%A_Index%
        planterField := PlanterField%A_Index%
        planterHarvestTime := PlanterHarvestTime%A_Index%
        planterNectar := PlanterNectar%A_Index%
        planterEstPercent := PlanterEstPercent%A_Index%
        
        if (planterName && planterName != "None") {
            timeRemaining := Max(0, planterHarvestTime - nowUnix())
            statusData.planters.Push({
                slot: A_Index,
                name: planterName,
                field: planterField ? planterField : "Unknown Field",
                harvestTime: planterHarvestTime,
                timeRemaining: timeRemaining,
                nectar: planterNectar ? planterNectar : "Unknown",
                growthPercent: planterEstPercent ? planterEstPercent : 0,
                status: timeRemaining <= 0 ? "Ready to Harvest" : "Growing",
                minutesLeft: timeRemaining > 0 ? Round(timeRemaining / 60) : 0
            })
        } else {
            statusData.planters.Push({
                slot: A_Index,
    ...
    try {
        FileAppend "[" A_Now "] Sending Status Data:`n", "debug_hive_api.log"
        FileAppend "- Message: " statusMessage "`n", "debug_hive_api.log"
        FileAppend "- Current Field: " (statusData.current.field ? statusData.current.field : "MISSING") "`n", "debug_hive_api.log"
        FileAppend "- Backpack: " (statusData.current.backpackPercent ? statusData.current.backpackPercent : "MISSING") "`n", "debug_hive_api.log"
        FileAppend "- Session Honey: " (statusData.session.totalHoney ? statusData.session.totalHoney : "MISSING") "`n", "debug_hive_api.log"
        FileAppend "- Planters Count: " statusData.planters.Length "`n", "debug_hive_api.log"
        FileAppend "---`n", "debug_hive_api.log"
    } catch {
        ; Silent fail on debug logging
    }
    
    ; Send comprehensive data to Hive AI
    nm_SendHiveData("status", statusMessage, statusData)
}

; Send enhanced analytics data to Hive AI
nm_HiveAnalytics() {
    if (!WebTrackerEnabled)
        return
    
    ; GUARANTEE all global variables are accessible
    global serverStart, SessionTotalHoney, HoneyAverage, TotalRuntime
    global SessionBossKills, SessionBugKills, SessionViciousKills
    global SessionQuestsComplete, SessionPlantersCollected, SessionDisconnects
    global TotalBossKills, TotalBugKills, TotalViciousKills
    global TotalQuestsComplete, TotalPlantersCollected, TotalDisconnects
    global BackpackPercentFiltered, FieldBoostStacks, FieldLastBoosted
    global MoveMethod, MoveSpeedNum, HiveBees, HiveSlot
    global CurrentField, FieldName1, FieldPattern1
    
    ; Calculate session runtime - GUARANTEED to work
    sessionRuntime := (IsSet(serverStart) && serverStart) ? (nowUnix() - serverStart) : 0
    
    ; Create analytics message
    analyticsMessage := "Analytics Update - Runtime: " . DurationFromSeconds(sessionRuntime) . ", Performance Update"
    
    ; Prepare comprehensive analytics data with REAL variables
    analyticsData := {
        performance: {
            honeyPerHour: (sessionRuntime > 3600 && SessionTotalHoney) ? Round(SessionTotalHoney / (sessionRuntime / 3600), 2) : 0,
            averageHoney: HoneyAverage ? HoneyAverage : 0,
            efficiency: (sessionRuntime > 0) ? Round((sessionRuntime * 0.85 / sessionRuntime) * 100, 1) : 0,
            gatheringSpeed: BackpackPercentFiltered > 0 ? Round(BackpackPercentFiltered / (sessionRuntime / 60), 2) : 0,
            avgFieldTime: sessionRuntime > 0 ? Round(sessionRuntime / 60, 1) : 0,
            conversionsPerHour: sessionRuntime > 3600 ? Round(1 / (sessionRuntime / 3600), 1) : 0
        },
        session: {
            startTime: serverStart ? serverStart : nowUnix(),
            runtime: sessionRuntime,
            totalHoney: SessionTotalHoney ? SessionTotalHoney : 0,
            sessionHoney: SessionTotalHoney ? SessionTotalHoney : 0,
            totalRuntime: TotalRuntime ? TotalRuntime : 0,
            conversions: 0, ; Will be tracked separately
            gatherTime: sessionRuntime ? Round(sessionRuntime * 0.85) : 0,
            convertTime: sessionRuntime ? Round(sessionRuntime * 0.15) : 0,
            bossKills: SessionBossKills ? SessionBossKills : 0,
            bugKills: SessionBugKills ? SessionBugKills : 0,
            viciousKills: SessionViciousKills ? SessionViciousKills : 0,
            questsComplete: SessionQuestsComplete ? SessionQuestsComplete : 0,
            plantersCollected: SessionPlantersCollected ? SessionPlantersCollected : 0,
            disconnects: SessionDisconnects ? SessionDisconnects : 0
        },
        boosts: {
            fieldBooster: FieldBoostStacks ? (FieldBoostStacks > 0) : false,
        ...
        settings: {
            moveMethod: MoveMethod ? MoveMethod : "Walk",
            moveSpeedNum: MoveSpeedNum ? MoveSpeedNum : 28,
            hiveBees: HiveBees ? HiveBees : 50,
            hiveSlot: HiveSlot ? HiveSlot : 6,
            currentField: CurrentField ? CurrentField : "No Field",
            fieldName1: FieldName1 ? FieldName1 : "No Field",
            fieldPattern1: FieldPattern1 ? FieldPattern1 : "No Pattern"
        }
    }
    
    ; Send to Hive AI
    nm_SendHiveData("analytics", analyticsMessage, analyticsData)
}

; Send specific BSS events to Hive AI
nm_HiveEvent(eventType, eventData := {}) {
    if (!WebTrackerEnabled)
        return
    
    ; Create event-specific messages
    eventMessage := ""
    switch eventType {
        case "conversion_start":
            eventMessage := "Converting - Returning to Hive"
        case "conversion_complete":
            eventMessage := "Conversion Complete - " . (eventData.honeyGained ? eventData.honeyGained : "Unknown") . " Honey Gained"
        case "field_change":
            eventMessage := "Field Change - Moving to " . (eventData.toField ? eventData.toField : "Unknown Field")
        case "boss_fight":
            eventMessage := "Boss Fight - Engaging " . (eventData.bossName ? eventData.bossName : "Unknown Boss")
        case "boss_defeat":
            eventMessage := "Boss Defeated - " . (eventData.bossName ? eventData.bossName : "Unknown Boss")
        case "quest_complete":
            eventMessage := "Quest Complete - " . (eventData.questName ? eventData.questName : "Unknown Quest")
        case "planter_plant":
            eventMessage := "Planter Planted - " . (eventData.planterName ? eventData.planterName : "Unknown Planter")
        case "planter_harvest":
            eventMessage := "Planter Harvested - " . (eventData.planterName ? eventData.planterName : "Unknown Planter")
        case "boost_use":
            eventMessage := "Boost Used - " . (eventData.boostName ? eventData.boostName : "Unknown Boost")
        case "disconnect":
            eventMessage := "Disconnected - Reconnecting to Roblox"
        case "reconnect":
            eventMessage := "Reconnected - Resuming Macro"
        case "macro_start":
            eventMessage := "Macro Started - Beginning Session"
        case "macro_stop":
            eventMessage := "Macro Stopped - Session Ended"
        default:
            eventMessage := "Event - " . eventType
    }
    
    ; Add eventType to the data being sent
    if (!IsObject(eventData)) {
        eventData := {}
    }
    eventData.eventType := eventType
    eventData.timestamp := nowUnix()
    
    ; Send event data
    nm_SendHiveData("event", eventMessage, eventData)
}

; Send conversion tracking with REAL data
nm_HiveConversion(pollenAmount, expectedHoney, fieldSource) {
    ; GUARANTEE global variables
    global serverStart, BackpackPercentFiltered, SessionTotalHoney, HiveBees
    
    sessionRuntime := (IsSet(serverStart) && serverStart) ? (nowUnix() - serverStart) : 0
    nm_HiveEvent("conversion_start", {
        pollenAmount: pollenAmount,
        backpackFull: BackpackPercentFiltered >= 95,
        backpackPercent: BackpackPercentFiltered ? BackpackPercentFiltered : 0,
        fieldSource: fieldSource,
        expectedHoney: expectedHoney,
        conversionTime: 45,
...
; Send field change tracking with REAL data
nm_HiveFieldChange(fromField, toField, reason := "Unknown") {
    ; GUARANTEE global variables
    global serverStart, BackpackPercentFiltered, HiveBees
    
    sessionRuntime := (IsSet(serverStart) && serverStart) ? (nowUnix() - serverStart) : 0
    nm_HiveEvent("field_change", {
        fromField: fromField,
        toField: toField,
        pollenCollected: BackpackPercentFiltered ? (BackpackPercentFiltered . "% Pollen") : "0% Pollen",
        timeSpent: sessionRuntime > 0 ? Round(sessionRuntime / 60, 1) : 0,
        efficiency: (sessionRuntime > 0) ? Round((sessionRuntime * 0.85 / sessionRuntime) * 100, 1) : 0,
...
; Test Hive AI connection
nm_TestHiveAI() {
    if (!WebTrackerEnabled || !WebTrackerToken || WebTrackerToken = "bz_your_generated_key_here") {
        MainGui["HiveStatus"].Text := "⚠️ Please enable tracking and enter your API key"
        MsgBox "Please enable web tracking and enter your API key first!", "API Test", 0x40010
        return false
    }
    
    ; Show testing status
    MainGui["HiveStatus"].Text := "🔄 Testing connection..."
    
    testMessage := "Connection test from Natro Macro v" . VersionID
    testData := {
        test: true,
        version: VersionID,
        timestamp: nowUnix(),
    ...
    if (nm_SendHiveData("test", testMessage, testData)) {
        MainGui["HiveStatus"].Text := "✅ Connection successful! Ready to track your macro"
        MsgBox "Connection successful!`n`nYour macro is now linked to your web dashboard.", "API Test", 0x40040
        return true
    } else {
        MainGui["HiveStatus"].Text := "❌ Connection failed - Check your API key and internet"
        MsgBox "Connection failed!`n`nPlease check your API key and internet connection.", "API Test", 0x40010
        return false
    }
}
...
nm_importConfig()
{
	...
	config["Settings"] := Map("GuiTheme", "MacLion3"
		...
		, "TimersHotkey", "F5"
		, "WebTrackerEnabled", 1
		, "WebTrackerURL", "https://mthwqwussjcxrjjyzaja.supabase.co/functions/v1/hive-ai"
		, "WebTrackerToken", "bz_58389kdkntw7diyxexemjl"
		, "WebTrackerUpdateInterval", 60
		, "WebTrackerRetryCount", 3
		, "WebTrackerTimeout", 10000
		, "ShowOnPause", 0
		, "IgnoreUpdateVersion", ""
		, "FDCWarn", 1
...
; Include bitmap files with error handling (optional for functionality)
try {
    #Include "%A_ScriptDir%\..\nm_image_assets\"
    #Include "%A_ScriptDir%\..\nm_image_assets\general\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\gui\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\beemenu\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\buffs\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\convert\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\collect\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\kill\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\boost\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\inventory\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\reconnect\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\fdc\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\offset\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\perfstats\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\gui\blendershrine_bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\quests\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\sprinkler\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\stickerstack\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\stickerprinter\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\memorymatch\bitmaps.ahk"
    #Include "%A_ScriptDir%\..\nm_image_assets\reset\bitmaps.ahk"
} catch Error as e {
    ; Bitmap files are optional - macro can work without them
}
...
; add tabs
TabArr := ["Gather","Collect/Kill","Boost","Quests","Planters","Status","Settings","API","Misc","Credits"], (BuffDetectReset = 1) && TabArr.Push("Advanced")
...
TabCtrl.UseTab("API")
MainGui.SetFont("w700")
MainGui.Add("GroupBox", "x10 y30 w480 h180", "🐝 Hive AI Web Tracker")
MainGui.SetFont("s8 cDefault Norm", "Tahoma")

; Main enable checkbox
(GuiCtrl := MainGui.Add("CheckBox", "x20 y50 vWebTrackerEnabled Checked" WebTrackerEnabled, "Enable Web Tracking")).Section := "Settings", GuiCtrl.OnEvent("Click", nm_HiveToggle)

; API Configuration
MainGui.Add("Text", "x20 y75 w100", "API Configuration:")
MainGui.Add("Text", "x20 y95", "API Key:")
(GuiCtrl := MainGui.Add("Edit", "x80 y92 w300 h20 vWebTrackerToken", WebTrackerToken)).Section := "Settings", GuiCtrl.OnEvent("Change", nm_HiveSaveConfig)
MainGui.Add("Button", "x390 y92 w50 h20 vHiveTest", "Test").OnEvent("Click", nm_TestHiveAI)

; Status display
MainGui.Add("Text", "x20 y120 w80", "Status:")
MainGui.Add("Text", "x80 y120 w200 h20 vHiveStatus", WebTrackerEnabled ? "✅ Ready - Data will sync when macro starts" : "❌ Disabled - Check box above to enable")

; Information
MainGui.Add("Text", "x20 y145 w460 h60 +Wrap", "This connects your macro to a web dashboard where you can monitor your bee swarm simulator automation from anywhere. Get your API key from your dashboard website, enter it above, and click Test to verify the connection.")

SetLoadingProgress(26.5)
...
nm_Start(){
	...
	global LastWebUpdate
	Loop {
		; Send periodic analytics update (every 60 seconds) with ERROR HANDLING
		try {
			if (WebTrackerEnabled && IsSet(LastWebUpdate) && (nowUnix() - LastWebUpdate) >= 60) {
				nm_HiveAnalytics()
				LastWebUpdate := nowUnix()
			} else if (WebTrackerEnabled && !IsSet(LastWebUpdate)) {
				; First time - initialize LastWebUpdate
				LastWebUpdate := nowUnix()
			}
		} catch {
			; If analytics fails, don't crash the macro
			LastWebUpdate := nowUnix()
		}
...
nm_setStatus(newState:=0, newObjective:=0){
	...
	; Send to Hive AI web tracker
	if (WebTrackerEnabled && (newState != "Detected")) {
		nm_HiveStatus(newState ? newState : state, newObjective ? newObjective : objective)
	}
...
nm_convert(){
	...
	; Send conversion start tracking to Hive AI with REAL data
	if (WebTrackerEnabled && BackpackPercentFiltered > 0) {
		currentFieldName := CurrentField ? CurrentField : (FieldName1 ? FieldName1 : "No Field")
		expectedHoney := SessionTotalHoney ? Round(SessionTotalHoney * (BackpackPercentFiltered / 100), 2) : "Calculating"
		nm_HiveConversion(BackpackPercentFiltered . "% Full", expectedHoney, currentFieldName)
	}
...
start(*) {
	...
	;load web tracker config and send startup event
	nm_LoadWebTrackerConfig()
	; DEBUG: Log config loading
	try {
		FileAppend "[" A_Now "] WebTracker Config Loaded:`n", "debug_hive_api.log"
		FileAppend "- Enabled: " WebTrackerEnabled "`n", "debug_hive_api.log"
		FileAppend "- URL: " WebTrackerURL "`n", "debug_hive_api.log"
		FileAppend "- Token: " (WebTrackerToken ? "SET" : "MISSING") "`n", "debug_hive_api.log"
		FileAppend "---`n", "debug_hive_api.log"
	} catch {
		; Silent fail on debug logging
	}
...
; ===================================
; HIVE AI WEB TRACKER GUI FUNCTIONS
; ===================================

; Toggle web tracker on/off
nm_HiveToggle(GuiCtrl, *) {
    global WebTrackerEnabled
    ; Use standard config saving
    nm_saveConfig(GuiCtrl)
    
    ; Update status text
    statusText := WebTrackerEnabled ? "✅ Ready - Data will sync when macro starts" : "❌ Disabled - Check box above to enable"
    MainGui["HiveStatus"].Text := statusText
    
    ; Enable/disable other controls
    MainGui["WebTrackerToken"].Enabled := WebTrackerEnabled
    MainGui["HiveTest"].Enabled := WebTrackerEnabled
    
    ; Send status update if enabling
    if (WebTrackerEnabled && WebTrackerToken && WebTrackerToken != "bz_your_generated_key_here") {
        nm_HiveEvent("tracking_enabled", {enabled: true})
    }
}

; Save web tracker configuration
nm_HiveSaveConfig(GuiCtrl, *) {
    global WebTrackerToken, WebTrackerEnabled
    ; Use standard config saving
    nm_saveConfig(GuiCtrl)
    
    ; Update status text based on current state
    if (WebTrackerToken && WebTrackerToken != "bz_your_generated_key_here") {
        if (WebTrackerEnabled) {
            MainGui["HiveStatus"].Text := "✅ Ready - Data will sync when macro starts"
        }
    } else {
        MainGui["HiveStatus"].Text := "⚠️ Enter your API key above"
    }
}
